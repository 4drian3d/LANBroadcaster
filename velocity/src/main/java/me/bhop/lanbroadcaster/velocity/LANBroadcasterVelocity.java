...
public class LANBroadcasterVelocity {
  ...
  public void onProxyInit(ProxyInitializeEvent event) {
    try {
      final InboundConnection inboundConnection = new LANInboundConnection(proxyServer.getBoundAddress());
      this.broadcaster = LANBroadcaster.initialize(
          proxyServer.getBoundAddress().getPort(),
          new VelocityMOTDProvider(proxyServer, proxyServer.getEventManager(), inboundConnection),
          logger);
